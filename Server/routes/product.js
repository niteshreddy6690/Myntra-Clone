...
router.get("/:combineCategory", async (req, res) => {
  ...
  // Deleting the Gender property from the object QueryObject
  var remainingQueryObject = Object.assign({}, queryObject);
  if (
    combineCategory.trim.toLocaleLowerCase == "women" ||
    combineCategory.trim.toLocaleLowerCase == "men" ||
    combineCategory.trim.toLocaleLowerCase == "boys" ||
    combineCategory.trim.toLocaleLowerCase == "girls"
  ) {
...
router.get("/search/autosuggest", async (req, res) => {
  ...
  //
  res.status(200).json({
    searchBrand: [...new Set(searchBrand.map((item) => item?.brand))],
    searchCategory: [...new Set(searchCategory)],
  });
});
